...
def yay0Dec(data):
	...
	log.debug ("Link table at", o_lt, "bytes and byte-chunks etc starting at", o_bccm, "bytes.")
	...
	while(o_dest < decoded_size):
		...
		mask_count = 32
		log.debug("Mask:", bin(mask), hex(mask), "at", o_mt)
		o_mt += 4
		log.debug("Moved o_mt to", o_mt)
		...
		log.debug(mask_count, ":Getting next mask")
	log.info("Done decoding to", o_dest, "bytes.")
	return dest
